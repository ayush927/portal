...
echo "<br>IQ Percentile</br>";
echo $iq_cpm_percentile;
echo "<br>";
...
function checking_size($logo,$pdf)
{
    ...
    if($wImg<=512 && $hImg<=512)
    {
        $pdf->SetXY(175, 8);
        $pdf->SetFont('arial');
        $pdf->Cell(0,20,$pdf->Image($logo,$pdf->GetX(), $pdf->GetY(),25), 0, 0,'R',false);
    }
    else
    {
        $pdf->SetXY(150, 10);
...
echo "<br>OK till line 120<br>";
 $pagecount = $pdf->setSourceFile('report_template/Cat_CBI_Page.pdf');
//$pagecount = $pdf->setSourceFile('report_template/Cat_CBI_Page_old.pdf');

echo "<br>OK till line 123<br> ";
