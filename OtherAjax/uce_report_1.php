...
// echo "<pre>";
// print_r($_SERVER );
$sign_url = ($_SERVER['SERVER_PORT'] == 443 ? 'https' : 'http') . "://{$_SERVER['SERVER_NAME']}/portal/";
// die;
ob_start();
...
$pdf->AddPage();
$pdf->useTemplate($tpl, ['adjustPageSize'=>true]);
//report download status change in user_code_list
// $sql_for_report_status = "update user_code_list set report_status='1' where code='$code'";
...
//remark and date
$c_remark=$row_for_logo['c_remark'];
$remark_date=$row_for_logo['remark_update_last'];
...
// echo
// echo "<pre>";
// print_r( $row_for_reseller_info );
$sign = $row_for_reseller_info['reseller_signature'];
// echo $sign;
// die;
if($sign){
    $signature = $sign_url.$sign;
}
$logo_orig = $logo =  $row_for_reseller_info['logo'];
...
//if($logo_orig){echo "in if logo";die();}else{echo "in else logo";die();}
...
$J = 0;
    ...
    $tpl = $pdf->importPage(10);//page 9 of report
    if($logo_orig){checking_size($logo,$pdf);}
    $pdf->AddPage();
    $pdf->useTemplate($tpl, ['adjustPageSize'=>true]); 
...
// Interest Mapper
    ...
    while($row_ft=mysqli_fetch_array($res_ft))
    {
        ...
        //echo "uce_report_v1"."<br>".__LINE__."<br>";die();
...
ob_end_clean();
$pdf->AliasNbPages();
$pdf->Output('F', '../assets/report-pdf/' .$code.".pdf", true);
...
$sql_for_report_status = "update report_request set status = 1 , file_name = '".$code.".pdf' where code='$code'";
$res_for_logo = mysqli_query($con,$sql_for_report_status);
$pdf->Output();
