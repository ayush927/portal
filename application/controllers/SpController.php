...
class SpController extends CI_Controller
{
    ...
    public function edit_sp_profile()
    {
        ...
        if(isset($_POST['updatebtn']))
        {
            $this->form_validation->set_rules('full_name','Fullname|min_length[3]','required');
             $this->form_validation->set_rules('profile_link','Profilelink','required|is_unique[user_details.profile_link]');
            ...
            {   
                ...
                if( isset( $errors['profile_link'] ) ){
                    $this->session->set_flashdata('msg2','Profile Link must be unique');
                }
                else{
    ...
    // Add by new function by Danish #START here.
      public function page_change_logo()
        ...
        public function user_buyer($request_generate = null)
        {
            ...
            $code['reportControl'] = $this->User_model->getreportcontrol($user['id']);
        ...
        public function create_request($code, $solution, $user_id)
        {
            $this->load->model('Admin_model');
            $this->load->model('User_model');
            ...
            if($code != '' && $solution != '' && $user_id != ''){
                ...
                if( $id ){
                    redirect(base_url().'OtherAjax/download_report.php?code='.base64_encode($code));
                    $this->session->set_flashdata('msg' , 'Request generated');
                    // redirect('/UserController/user_buyer','refresh');    
                }
                else{
                    $this->session->set_flashdata('msg' , 'Request Not generated, Try again');
                    redirect('/SpController/user_buyer','refresh');
                }
            }
            else{
        ...
        public function set_report_control($control){
            $this->load->model('User_model');
            $user = $this->session->userdata('user');
            $this->User_model->setControlUpdate( [ 'status' => $control ], [ 'user_id' => $user['id'] ] );
        ...
        public function set_mannual_control($control , $code){
            $this->load->model('User_model');
            $user = $this->session->userdata('user');
            $this->User_model->setControlUpdateReport( [ 'mannual_status' => $control ], [ 'code' => $code ] );
            // echo $this->db->last_query();
            // die;
            if( $this->db->affected_rows() ){
                $this->session->set_flashdata('msg' , 'Report View Status is updated');
                redirect('/SpController/user_buyer','refresh');
            }
            else{
    ...
    public function marketplace_new($num=null)
    {
        $key = $this->input->post('levelone');
        $key1 = $this->input->post('leveltwo');
        $key2 = $this->input->post('levelthree');
        $key = $key?$key:'';
        $key1 = $key1?$key1:'';
        $key2 = $key2?$key2:'';
        $key3 =array(
        ...
        $data['lk'] = $this->Admin_model->provider_level_list();
        $data['sk'] = $this->Admin_model->provider_level3_list();
        $config = array();
        $config["base_url"] = base_url() . "service-providers";
        if( empty($_POST) ){
            $config["total_rows"] = $this->Admin_model->record_count(['iam' => 'sp' , 'status!=' => 0 , 'id>=' => 1]);
        }
        elsE{
            $config["total_rows"] = count($this->Admin_model->fetch_user_details("" , "" ,$key3) );
        }
        ...
        // echo $page."<br>";
        $data["l"] = $this->Admin_model->fetch_user_details($config["per_page"], ((($page == 0 ? 1 : $page)-1)*10),$key3);
        ...
        $this->load->view("sp/sp_list",$data);
        $this->load->view("sp/sp_footer",$data);
    }
...
public function section_via_parameter()
    ...
    {       
        if($this->User_model->authorized()==false)
        {
            ...
            //$this->load->view('sidebar',$data);
            $this->load->view('sp/sidebar',$data); 
            $this->load->view('sp/add_associate'); 
            $this->load->view('footer');
    }

    public function insertAssociate()
    {   
        if($this->User_model->authorized()==false)
        {
            $this->session->set_flashdata('msg','You are not authorized to access this section');
        ...
        $num_rows = $this->Sp_model->get_num_rows_for_associate($user['user_id']);
        $next_num=(int)$num_rows+1;
        $arr_u_id= ['ecom'=>'iquery'];
        
        $uid=$arr_u_id[$user['user_id']];
        $associate_type=$this->input->post('associate_type');
        if($associate_type=='Internal'){
            $other_code=$uid."-int-".$next_num;
        }else{
            $other_code=$uid."-ext-".$next_num;
        }
        // echo $other_code."<br>";
        // print_r($user['user_id']);die();
        
        $formData = array();
        $formData['associate_type'] = ($this->input->post('associate_type'))?$this->input->post('associate_type'):'';
        $formData['email'] = ($this->input->post('email'))?$this->input->post('email'):'';
        $formData['selected_code'] = ($this->input->post('selected_code'))?$this->input->post('selected_code'):'';
        $formData['code'] = $code;
        $formData['other_code'] = $other_code;
        $formData['user_id'] = $user['user_id'];
        $formData['user_type'] = $user['iam'];
        ...
        {  
            ...
            redirect('/SpController/getAllAssociate/','refresh');
    ...
    public function insert_selected_code()
    {   
        if($this->User_model->authorized()==false)
        {
            $this->session->set_flashdata('msg','You are not authorized to access this section');
        ...
        $user = $this->session->userdata('user');
        //$formData = array();
        $selected_code = ($this->input->post('selected_code'))?$this->input->post('selected_code'):'';
        $email = $user['email'];
        $user_id = $user['user_id'];
        $user_type = $user['iam'];
 
        $result = $this->Sp_model->insert_selected_code($selected_code,$email,$user_id,$user_type ); 
        //echo "res=".$result;die();
        if($result)
        {  
            $this->session->set_flashdata("msg2","Added Successfully");
    ...
    {   
        ...
        $db_data['associate_data'] = $this->Sp_model->getAssociate($user_id,$user_type ); 
        // echo "<pre>";
        // print_r($db_data);die();
        $data = $this->initializer();
        ...
        //$this->load->view('sidebar',$data);
        $this->load->view('sp/sidebar',$data); 
        $this->load->view('sp/view_associate',$db_data); 
        $this->load->view('footer');
    ...
    {   
        ...
        //$this->load->view('sidebar',$data);
        $this->load->view('sp/sidebar',$data); 
        $this->load->view('sp/edit_associate',$db_data); 
        $this->load->view('footer');

    }


    public function updateAssociate($id)
    {   
        if($this->User_model->authorized()==false)
        {
            $this->session->set_flashdata('msg','You are not authorized to access this section');
        ...
        $formData['associate_type'] = ($this->input->post('associate_type'))?$this->input->post('associate_type'):'';
        $formData['email'] = ($this->input->post('email'))?$this->input->post('email'):'';
        $formData['code'] = ($this->input->post('code'))?$this->input->post('code'):'';
        $formData['selected_code'] = ($this->input->post('selected_code'))?$this->input->post('selected_code'):'';
        $formData['user_id'] = $user['user_id'];
        $formData['user_type'] = $user['iam'];
 
        $result = $this->Sp_model->updateAssociate($formData,$id); 
    ...
    {   
        ...
        $id=base64_decode($id);
        $result = $this->Sp_model->deleteAssociate($id); 
        ...
        {
            $this->session->set_flashdata("msg","Opps Unable to delete data");
            redirect('/SpController/getAllAssociate/','refresh');
        } 

    }
    
    public function request_for_retest($id,$group)
        {   
            if($this->User_model->authorized()==false)
            {
                $this->session->set_flashdata('msg','You are not authorized to access this section');
            ...
            $grp= base64_decode($group);
            if($grp=='Career Explorer'){$sol='ceot';}
            elseif($grp=='Positive Parenting'){$sol='ppot';}
            elseif($grp=='Career Excellence'){$sol='cexot';}
            elseif($grp=='Career Builder'){$sol='cbot';}
            elseif($grp=='Overseas Companion'){$sol='ocoxt';}
            // echo $sol;die();
            // die();
            $user = $this->session->userdata('user');
            $email_id = $user['email'];
            ...
            $this-> db->where('solution', $sol);
            $this-> db->delete('ppe_part1_test_details');
            
            $this->db->where('id',$id);
            $this->db->set('status','ua');
            $this->db->update('partner_counselor_status');
            $this->session->set_flashdata('msg  ','Request send successfully Please wait for approval.');
            redirect(base_url().'SpController/become_partner_counselor');
        }
