...
class RazorpayController extends CI_Controller {
  ...
  public function initializer(){
    ...
    public function insertCreditial($id = null)
    {   
            $this->load->model('User_model');
            $this->load->model('Admin_model');
            if($this->User_model->authorized()==false)
            ...
            $data['get_payment_details_id_vias'] = $this->Admin_model->get_payment_details_id_vias($id,$user['id']);
            ...
            if(isset($_POST['updatebtn']))
            {
                ...
                {  
                    $count = $this->Admin_model->get_payment_details_id_vias( null , $user['id']);
                     $formArray = array(
    ...
    public function razorypayStatus(){
        ...
        $user = $this->session->userdata('user');
        $tt = $this->Admin_model->getPaymentGatway($id);
        $allowed_services = $this->Admin_model->getUserDetailsById($user['id']);
        //   echo  "<pre>";print_r($allowed_services);die;
        $rr = explode(",",$allowed_services); 
        if(!in_array("payment_gateways", $rr) && $tt->row_array() > 1 ){
            die("heelo1");
            $status = ($status==0) ? 1:0;
