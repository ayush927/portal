<?php 
    class AdminController extends CI_Controller
    {
        ...
        public function login_page_design($design_type)
        {   
            if($this->User_model->authorized()==false)
            {
                $this->session->set_flashdata('msg','You are not authorized to access this section');
            ...
            $data['logindata'] = $this->Admin_model->getloginPageDesignData($type);
            $data['calendly_url']='https://calendly.com/';
            $this->load->view('navbar',$data);
            $data['type'] = $type;
            $this->load->view('admin/sidebar',$data);
            $this->load->view('admin/login_page_design' , $data);
            $this->load->view('footer');
        }
        ...
        {   
            ...
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar',$data);
            $this->load->view('admin/report_config' , $data);
            $this->load->view('footer');
        }
        
        function add_report_data(){
            if( !empty( $_POST ) ){
                $this->Admin_model->updateReportData( $_POST );
                redirect( 'AdminController/report-config' );
            }
            else{
                redirect( 'AdminController/report-config' );
            }
        ...
        function add_login_form_detail(){
            if( !empty( $_POST ) ){
                if( isset( $_POST['design_type'] ) ){
                    ...
                    unset( $_POST['design_type'] );
                }
                foreach( $_POST as $key => $value ){
                    if( $key == 'background_type' && $value == 1 ){
                        if(  $_FILES['background_image']['name'] != '' ){
                            $config['upload_path'] = './uploads/';
                            $config['allowed_types'] = 'jpg|png|jpg|jpeg';
                            $config['max_size'] = 100000;
                            $dname = explode(".", $_FILES['background_image']['name']);
                            $ext = end($dname);
                            $new_name = time().'.'.$ext;
                            ...
                            if ( ! $this->upload->do_upload('background_image'))
                            {
                                ...
                                foreach($error as $error)
                                {
                                    ...
                                    redirect('AdminController/login-page-design','refresh');
                                }
                            }
                            else
                            {
                                $this->Admin_model->updateloginform( $type, 'background_image' , 'uploads/'.$new_name );
                            }
                        }
                        else{
                            $this->session->set_flashdata('msg','No file selected');
                            redirect('AdminController/login-page-design','refresh');
                        }
                        $this->Admin_model->updateloginform( $type , $key , $value );
                    }
                    else{
                        $this->Admin_model->updateloginform( $type,  $key , $value );
                    }
                ...
                redirect( 'AdminController/login-page-design/'.$sendTo );
            }
            else{
                redirect( 'AdminController/login-page-design/'.$sendTo );
            }
        ...
        public function code_approval($id, $check = null)
        {
            ...
            if($this->session->userdata('isnotloggedin') ){
                ...
                if( $check != null ){
                    ...
                    $this->db->where(["reseller_id"=> $parentData['email'], 'user_id' => $user_details['email'] ,"status"=>"pending"]);
                    ...
                    // print_r( $solution_details );
                    // print_r( $row );
                    // die;
                    redirect(base_url().'UserController/code_approvel_for_user/'.$row['id']."/".$row['solution']."/".$parentData['email']."/".$check);
                }
                else{
        ...
        public function serviceConfig($id)
        {
            $this->load->model('User_model');
            if($this->User_model->authorized()==false)
            {
            ...
            $this->load->model('Admin_model');
            $spResellers['services'] = $this->Admin_model->getUserDetailsById($id);
            $spResellers['id'] =$id;
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
            $this->load->view('admin/service_config_view',$spResellers);
            $this->load->view('footer');
        }
        ...
        public function edit_services_detail()
        {
            $this->load->model('Admin_model');
            if($this->input->is_ajax_request()){
                $edit_id = $this->input->post('edit_id');
                if($post = $this->Admin_model->edit_detail($edit_id)){
                    $data = array('responce' => 'success', 'post' => $post);
                }
                else
                {
        ...
        public function update_services_data()
        {
            if(isset($_POST['update']))
            {
                ...
                $this->form_validation->set_rules('retail_price','Retail Discount','required');
                $this->form_validation->set_rules('retail_discount','Retail Discount','required');
                ...
                if($this->form_validation->run() == true)
                {
                    ...
                    $rprice= $_POST['retail_price'];
                    $rdisc= $_POST['retail_discount'];
                    ...
                    else
                    {
                        ...
                        $this->db->where('code_id',$code_id);
                        $this->db->update('reseller_code_record');
                        $this->db->set('retail_price',$rprice);
                        $this->db->set('retail_discount',$rdisc);
                        $this->db->where('code_id',$code_id);
        ...
        public function update_services_detail()
        {
            // print_r( $_POST );
            // die;
            if(isset($_POST['update']))
            {
                $this->form_validation->set_rules('edit_mrp','MRP','required');
                $this->form_validation->set_rules('edit_retailmrp','Retail Mrp','required');
                $this->form_validation->set_rules('edit_retaildic','Retail Discount','required');
                
                if($this->form_validation->run() == true)
                {
                    $id = $_POST['edit_id'];
                    $mrp= $_POST['edit_mrp'];
                    $rmrp= $_POST['edit_retailmrp'];
                    $rdisc= $_POST['edit_retaildic'];
                        $this->db->where('id',$id);
                        $this->db->set('mrp',"INR ".$mrp."/- (MRP)");
                        $this->db->set('mr_price',$mrp);
                        $this->db->set('retail_price',$rmrp);
                        $this->db->set('retail_discount',$rdisc);
                        $this->db->update('solutions');
                        $this->session->set_flashdata("msg","Details Updated");
                        $this->session->set_flashdata("status","success");
                        redirect('/AdminController/view_service_detail','refresh');
                    }
                    else{
                        $this->session->set_flashdata("msg","Details Not Updated Right");
        ...
        public function delete_services($id)
        {
         ...
         public function allowed_services_update($id,$services)
        {
           //echo $id;die;
            //$services = $this->input->post('configer');
            //echo $services;die;
            $this->load->model('Admin_model');
            ...
            if($userData!=''){
                ...
                $str = $userData;
                 $rr = explode(",",$str);
                if (in_array($services, $rr)){
                    if (($key = array_search($services, $rr)) !== false) {
                        unset($rr[$key]);
                        $allow = implode(",",$rr);
                    }
                  $this->Admin_model->allowed_services_update($id,$allow);
                }else{
                 $this->Admin_model->allowed_services_update($id,$userData.','.$services);
                }
               
            }else{
                $this->Admin_model->allowed_services_update($id,$services);
            }
         ...
         public function allowed_services_offline($id)
        {
           // echo $id;die;
            $this->load->model('Admin_model');
            $userData = $this->Admin_model->getUserDetailsById($id);
            $tt = $this->Admin_model->getPaymentGatway();
           // echo count($tt);die;
            if ( count($tt) > 0) {
               // $this->session->set_flashdata('msg','one setting is on');
            ...
            //print_r($userData);die;
           // $this->session->set_flashdata('msg','one setting is ondgdf');
             redirect('/payment-gateway/configure','refresh');
        }
        public function edit_reseller_status()
        {
            ...
            $this->db->where('id',$id);
            $this->db->update('user_details');
            $this->Admin_model->update_status( $id , $detailstatus );
            $data = array('responce'=>'success','message'=>'Data update Successfully');
            echo json_encode($data);
        }
        public function edit_reseller_page_status()
        {
            
            
            $id = $this->input->post('s');
            $detailstatus = $this->input->post('act');
            $this->Admin_model->update_status( $id , $detailstatus );
            $data = array('responce'=>'success','message'=>'Data update Successfully');
            echo json_encode($data);
        }
        ...
        public function view_service_detail()
        {
            ...
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
            $this->load->view('admin/view_service_detail',$solution);
            $this->load->view('footer');
        ...
        public function add_cluster($id = null)
        {           
            $this->load->model('User_model');
            if( $this->User_model->authorized() == false )
            {
            ...
            $user = $this->session->userdata('user');
            if( $id != null ){
                $data['edit'] = getQuery([ 'table' => 'clusters' , 'where' => [ 'id' => $id ] , 'single' => true ]);
            }
            $data['user'] = $user;
            $this->load->model('Admin_model');
            $this->load->view('navbar',$data);
        ...
        public function all_clusters(){
            ...
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
            $this->load->view('admin/all_clusters');
            $this->load->view('footer');
        }

        public function submit_cluster(){
            if(! isset($_POST['id']) ){
                $this->form_validation->set_rules('clustersName','Cluster Name','required');
            }
            else{
                $this->form_validation->set_rules('clustersName','Cluster Name','required|is_unique[clusters.clustersName]');
            }
            ...
            if($this->form_validation->run() == false){
                ...
                $this->session->set_flashdata('status' , 'danger');
                if( isset( $_POST['id'] ) ){
                    redirect( 'adminController/edit-cluster/'.$_POST['id'] , true );
                }
                else{
                    redirect( 'adminController/add-cluster' , true );
                }
            }
            else{
                if( isset( $_POST['id'] ) ){
                    $status = update( [ 'table' =>  'clusters' , 'data' =>  $_POST, 'where' => [ 'id' => $_POST['id'] ] ] );
                    // lQ(1);
                    if( affected() > 0 ){
                        $this->session->set_flashdata( 'msg' , 'Updated Career Cluster');
                        $this->session->set_flashdata('status' , 'success');
                        redirect( 'AdminController/all-clusters' , true );
                    }
                    else{
        ...
        function delete_clusters( $id ){
            $status = delete(['table' => 'clusters' , 'where' => [ 'id' =>  $id ] ] );
            if( $status == true ){
        ...
        public function add_career_cluster_details()
        {           
            $this->load->model('User_model');
            ...
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
            $this->load->view('admin/add_career_cluster_details');
            $this->load->view('footer');            
        }

        function status_career_content( $id, $status ){
            $data['status'] = $status;
            $status = $this->Admin_model->update_career_data( $data , $id );
            if( $status == true ){
                $this->session->set_flashdata( 'msg' , 'Updated Career Cluster');
                $this->session->set_flashdata('status' , 'success');
            }
        ...
        public function edit_cluster_detail($id)
        {           
            $this->load->model('User_model');
            ...
            $user = $this->session->userdata('user');
            $data['edit'] = $this->Admin_model->get_cluster_by_id($id);
            $data['user'] = $user;
            $this->load->model('Admin_model');
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
        ...
        function view_career_cluster_details(){
            $this->load->model('User_model');
            ...
            $user = $this->session->userdata('user');
            $data['user'] = $user;
            $data['list'] = $this->Admin_model->get_all_cluster();
            $this->load->model('Admin_model');
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
            $this->load->view('admin/view_career_cluster_details');
            $this->load->view('footer');
        }

        function delete_cluster_detail( $id ){
            $status = $this->Admin_model->delete_career_cluster($id);
            if( $status == true ){
                $this->session->set_flashdata( 'msg' , 'Deleted Career Cluster');
                $this->session->set_flashdata('status' , 'success');
                redirect( 'AdminController/view-career-cluster-details' , true );
            }
            else{
        ...
        function add_cluster_details(){
            if( !empty( $_POST ) ){
                $this->form_validation->set_rules("update_code","Update Code","trim|required");
                $this->form_validation->set_rules("profession_id","Profession Id","trim|required");
                $this->form_validation->set_rules("profession_name","Profession Name","trim|required");
                $this->form_validation->set_rules("profession_desc","Profession Description","trim|required");
                $this->form_validation->set_rules("profession_demand","Profession Demand","trim|required");
                if($this->form_validation->run() == false){
                    $this->session->set_flashdata( 'msg' , 'Please Fill up form Correctly');
                    $this->session->set_flashdata('status' , 'danger');
                    if( isset( $_POST['id'] ) ){
                        ...
                        // die;
                        redirect( 'AdminController/edit-cluster-content/'.$_POST['id'] , true );
                    }
                    else{
                        redirect( 'AdminController/add-cluster-content' , true );
                    }
                }
                else{
                    $_POST['update_time '] = date('m/d/Y H:1');
                    if( isset( $_POST['id'] ) ){
                        $status = $this->Admin_model->update_career_content_data( $_POST , $_POST['id'] );
                        if( $status == true ){
                            $this->session->set_flashdata( 'msg' , 'Updated Career Cluster Content');
                            $this->session->set_flashdata('status' , 'success');
                            redirect( 'AdminController/view-cluster-content' , true );
                        }
                        else{
                            $this->session->set_flashdata( 'msg' , 'Not Updated Try Again');
                            $this->session->set_flashdata('status' , 'info');
                        ...
                        $id = $this->Admin_model->insert_career_content_data( $_POST );
                        if( $id ){
                            $this->session->set_flashdata( 'msg' , 'Added Career Cluster Content');
                            $this->session->set_flashdata('status' , 'success');
                            redirect( 'AdminController/edit-cluster-content/'.$id , true );
                        }
                        else{
                            $this->session->set_flashdata( 'msg' , 'Something Error Haappen , Try Again');
                            $this->session->set_flashdata('status' , 'info');
                ...
                $this->session->set_flashdata('status' , 'danger');
                redirect( 'AdminController/add-career-cluster-details' );
            }
        }
        function add_cluster_content(){
            $this->load->model('User_model');
            ...
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
            $this->load->view('admin/add_cluster_content');
            $this->load->view('footer');
        }

        public function edit_cluster_content($id)
        {           
            $this->load->model('User_model');
            if($this->User_model->authorized()==false)
            {
            ...
            $user = $this->session->userdata('user');
            $data['edit'] = $this->Admin_model->get_cluster_content_by_id($id);
            $data['user'] = $user;
            $this->load->model('Admin_model');
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
        ...
        function view_cluster_content(){
            ...
            $data['user'] = $user;
            $this->load->model('Admin_model');
            $data['content'] = $this->Admin_model->view_cluter_content();
            $this->load->view('navbar',$data);
            $this->load->view('admin/sidebar');
            $this->load->view('admin/view-content', $data);
            $this->load->view('footer');
        }

        function status_cluster_content( $id, $status ){
            $data['status'] = $status;
            $status = $this->Admin_model->update_career_content_data( $data , $id );
            if( $status == true ){
                $this->session->set_flashdata( 'msg' , 'Updated Career Cluster');
                $this->session->set_flashdata('status' , 'success');
            }
        ...
        function insert_career_cluster() {
            if( !empty( $_POST ) ){
                ...
                if($this->form_validation->run() == false){
                    ...
                    if( isset( $_POST['id'] ) ){
                        // print_r( $this->form_validation->error_array() );
                        // die;
                        redirect( 'AdminController/edit-cluster-detail/'.$_POST['id'] , true );
                    }
                    else{
                        redirect( 'AdminController/add-career-cluster-details' , true );
                    }
                }
                else{
                    if( isset( $_POST['id'] ) ){
                        $status = $this->Admin_model->update_career_data( $_POST , $_POST['id'] );
                        if( $status == true ){
                            $this->session->set_flashdata( 'msg' , 'Updated Career Cluster');
                            $this->session->set_flashdata('status' , 'success');
                            redirect( 'AdminController/view-career-cluster-details' , true );
                        ...
                        $id = $this->Admin_model->insert_career_data( $_POST );
                        if( $id ){
                            $this->session->set_flashdata( 'msg' , 'Add Career Cluster');
                            $this->session->set_flashdata('status' , 'success');
                            redirect( 'AdminController/view-career-cluster-details' , true );
        ...
        function update_role_details() {
            if( !empty( $_POST ) ){
                if( isset( $_POST['id'] ) ){
                    ...
                    $this->session->set_flashdata('msg' , 'Please fill up the form');
                    $this->session->set_flashdata('status' , 'danger');
                    redirect( 'AdminController/add-cluster-content' );
                }
            }
            else{
                $this->session->set_flashdata('msg' , 'Please fill up the form');
        ...
        public function create_user_link(){
            ...
            if( !empty( $users ) ){
                foreach( $users as $key => $user ){
                    $users[$key]['profile_link'] = strtolower(str_replace(' ' , '-' , $user['fullname']));
                }
                // print_r( $users );
                // die;
                $this->Admin_model->update_batch_data('user_details' , $users);
                if( $this->db->affected_rows() > 0 ){
                    $this->session->set_flashdata('msg','Updated Profile Link');
                    redirect(base_url().'AdminController/view_reseller');
                }
            }
            else{
                $this->session->set_flashdata('msg','New SP/Reseller Found Added');
                redirect(base_url().'AdminController/view_reseller');
            }
        }
        ...
        public function sync_user(){
            ...
            if( !empty( $users ) ){
                foreach( $users as $key => $data ){
                    if( $data['iam'] == 'sp' ){
                        $sp_email  = $data['email'];
                        $getSpServicesName=$this->Admin_model->get_reseller_by_email($data['email']);
                        if( !empty($getSpServicesName['services']) ){
                            $insert[$index]['user_id'] = $users[$key]['id'];
                            $insert[$index]['services'] = $getSpServicesName['services'];
                            $insert[$index]['status'] = 'inactive';
                            $index++;
                        }
                        else{
                            $insert[$index]['user_id'] = $users[$key]['id'];
                            $insert[$index]['services'] = '';
                            $insert[$index]['status'] = 'active';
                            $index++;
                        }
                    }
                    else{
                        $getResellerData = $this->Admin_model->get_reseller_by_email($data['email']);
                        if( !empty($getResellerData['services']) ){
                            $insert[$index]['user_id'] = $users[$key]['id'];
                            $insert[$index]['services'] = $getResellerData['services'];
                            $insert[$index]['status'] = 'active';
                            $index++;
                        }
                        else{
                            $getSpData = $this->Admin_model->get_spdata_by_email($data['email']);
                            if( !empty( $getSpData['email'] ) ){
                                $serviceData = $this->Admin_model->select_service($getSpData['services']);
                                if( !empty($serviceData['services']) ){
                                    $insert[$index]['user_id'] = $users[$key]['id'];
                                    $insert[$index]['services'] = $serviceData['services'];
                                    $insert[$index]['services'] = $serviceData['services'];
                                    $insert[$index]['status'] = 'active';
                                    $index++;
                                }
                ...
                // die;
                if( !empty( $insert ) ){
                    $this->db->insert_batch('user_details_seo', $insert); 
                    $this->session->set_flashdata('msg','New SP/Reseller Found Added');
                    redirect(base_url().'AdminController/view_reseller');
                }
                else{
                    $this->session->set_flashdata('msg','No New SP/Reseller Found');
                    redirect(base_url().'AdminController/view_reseller');
                }
            }
        }
