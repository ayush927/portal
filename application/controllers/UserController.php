...

    {
...

        {
            ...
            if($checkKey != null){
                ...
                // redirect('BaseController/view_code/'.$solutionCode , 'refresh');
                    ...
                    if(password_verify($password,$user['pwd'])==true && $user['status']=='1' || password_verify($password,$user['pwd'])==true && $user['status']=='2'){
...

                        {
                            ...
                            if( $this->session->userdata('isnotloggedin') ){
                                ...
                                $this->db->where(["reseller_id"=> "merak@gmail.com", 'user_id' => $user['email'] ,"status"=>"pending"]);
                                ...
                                redirect(base_url().'UserController/code_approvel_for_user/'.$row['id']."/".$row['solution']."/merak@gmail.com");
                            ...
                            if( isset( $_GET['id']) ){
                                ...
                                if($_GET['id'] !=''){
                                    ...
                                    redirect(base_url().'BaseController/all_jobs?jobid='.$_GET['id']);  

                                }else  if($_GET['tid'] !=''){

                                    redirect(base_url().'BaseController/view_trainings?tid='.$_GET['tid']);
...

            {
                ...
                if( isset( $_POST['register_email_home'] ) ){
                    ...
                    if( !isset( $_POST['password'] ) ){
                        ...
                        $_POST['password'] = "SP".substr(str_shuffle($str_result), 0, 10);
                ...
                $this->form_validation->set_rules('serviceName','Service','required');
                    ...
                    // echo "3";
                        ...
                        if( !empty($userDetails) ){
                            ...
                            if( $userDetails['iam'] == 'user' ){
                                ...
                                $parentdata = $this->User_model->get_sp_details($user['user_id']);

                                if( !empty( $parentdata ) ){

                                    if( $parentdata['iam'] == 'sp' || $parentdata['iam'] == 'reseller' ){
...

                {
                    ...
                    if($this->session->userdata('isnotloggedin') ){
                        ...
                        if($check != null){
                            ...
                            redirect('AdminController/code_approval/'.$id.'/'.$check);
...

        {
            ...
            $q = $this->db->where(["user_id"=>$user["id"]])->order_by("id","desc")->get("user_book_appointment");
        ...
        public function code_approvel_for_user( $id, $solution, $email=null, $check=null )
...

                {
                        ...
                        if($this->session->userdata('isnotloggedin') ){
                            ...
                            if( $check != '' ){
                                ...
                                $codeData = $this->User_model->fetch_purchase_code_by_report($solution)->row_array();

                                // print_r( $codeData );

                                // die;

                                $purchaseCodeRequest = [ 'ccb1' => $codeData['code_id'] ,  'cb1' => 'request' ,  'purchase' => '' ];

                                $this->session->set_userdata( 'purchaseCodeRequest' , $purchaseCodeRequest);
...

        {
            ...
            // $code['reportStatus'] = $this->Base_model->reportData($row->solution,$row->code);

            $code['s'] = $this->User_model->code_list($user['email']);

            // echo "<pre>";

            // print_r( $code['s']->result_array() );
            ...
            switch($this->input->get_post('action')){
                ...
                case "FORGOT_PASSWORD":
                    ...
                    $OTP_code = rand(1000,1000000);
                        ...
                        if($this->User_model->otp_update($check_email_id['db_data']->id,$OTP_code) > 0){
                            ...
                            $subject = "Welcome from ".$this->domain." - Password Reset Request";

                            $html_msg  = "Dear ".$email_id." <br/> <br/> Please complete your registration with ".$this->domain."
                ...
                case "RESEND_OTP":
                    ...
                    if($check_user_status['num_rows'] > 0){
                        ...
                        $this->email->from( 'notification@respicite.com', 'astitvam.co.in');
