<?php
    class User_model extends CI_Model
    {
        ...
        public function getreportcontrol($id){
            $this->db->where('user_id',$id);
            return $row = $this->db->get('reseller_report_config')->row_array();
        }
        
        public function insertreportcontrol($formArray){
            $this->db->insert('reseller_report_config',$formArray);
        }
        public function get_sp_details($id = null){
            $this->db->where("user_id",$id);
        ...
        function setControlUpdate($data , $where)
        {
            $this->db->set($data);
            $this->db->where($where);
            $this->db->update('reseller_report_config');

        }
        function setControlUpdateReport($data , $where)
        {
            $this->db->set($data);
            $this->db->where($where);
        ...
        {
            $this->db->insert('report_request',$data);
            return $this->db->insert_id();
        }
        ...
        public function fetch_purchase_code_by_report($solution)
        {
            $where = "status='active' and solution='$solution' and no_of_reports = 1";
            $this->db->where($where);
            return $row = $this->db->get('reseller_code_record');
        }
        ...
        {
            $where = "user_code_list.reseller_id='$email' and user_code_list.status!='pending'";
            $this->db->select('user_code_list.*, generated_code_details.nature');
            $this->db->where($where);
            $this->db->join('generated_code_details' , 'generated_code_details.code = user_code_list.code' , 'left');
        ...
        public function get_data_by_email($email){
            $this->db->where('email',$email);
            $row = $this->db->get('user_details');
            return ["num_rows"=>$row->num_rows(),"db_data"=>$row->row_array()];
        }
        
        public function get_data_by_user_id($user_id){
            $sql="SELECT * FROM  user_details WHERE iam IN('sp','reseller') AND user_id = '".$user_id."'";    
        ...
        public function assessment_otp_update_by_email( $email, $otp){
            $this->db->set("assessment_otp", $otp);
            $this->db->where('email',$email);
            $this->db->limit(1);
            $this->db->update('user_details');
            return $this->db->affected_rows();  
        ...
        public function check_otp_email_reverify($email,$otp){
         ...
         public function check_assess_otp_email_reverify($email,$otp){
            $this->db->select("id");
            $this->db->where(['email'=>$email,"assessment_otp"=>$otp]);
            $user_id = $this->db->get('user_details');
            $db_res = "";
            if($user_id->num_rows() > 0){
                $this->otp_update($user_id->row()->id,null);
        ...
        //     //     // $this->email->from( $sendingEmail , 'respicite.com');
        //     //     $this->email->from( 'astitvam123@gmail.com' , 'astitvam');
        //     // }
        //     // else{
        ...
        //     //             'smtp_port' => 587,
        //     //             'smtp_user' => 'astitvam123@gmail.com',
        ...
        //             // $this->email->from('astitvam123@gmail.com', $_SERVER['SERVER_NAME']);
        //     $this->email->to($email);
        ...
        //     // $this->email->send();
        // }
        function otp_send_on_email( $email , $subject , $msg, $sendingEmail = null)
        {
            // echo $email;
            $this->load->library('email');
            $this->email->set_mailtype("html");
        ...
        {
            $this->db->select('*');
            $this->db->order_by('job_post_date');
        ...
        public function get_user_details($action = null,$id = null){
            switch($action){
                case "DETAILS_BY_EMAIL":
                    $this->db->select("id,user_id,iam,profile_photo,fullname,mobile,email");
                ...
                case "SOLUTION_BY_NAME":
                    $this->db->select("id,solution,display_solution_name,mr_price,retail_price,mrp");
                    $this->db->where("id",$id);
                ...
                case "EVENT_DETAIL":
                    // $this->db->select("id,solution,display_solution_name,mr_price,retail_price,mrp");
                    $this->db->where("id",$id);
                    $q = $this->db->get("admin_calendly_event");
                    // echo $this->db->last_query();
                    return $q->row_array();
                break;
            }
