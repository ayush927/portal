...
class Commen_model extends CI_Model
{
    ...
    **********************************/
    ...
    function get_distinct_cat($solution_table){    

        $this->db->select('category');
        $this->db->distinct();
        $query = $this->db->get($solution_table);
        return $query->result();
    }
    
    function get_interest_data($solution_table, $p1_2_cat){    

        $this->db->select('*');
        $this->db->from($solution_table);
        $this->db->where('category',$p1_2_cat);
        $query = $this->db->get();
        return $query->result();
    }
    
    function get_question_data($code, $solution_db, $qno){   

        $this->db->select('*');
        $this->db->from('ppe_part1_test_details');
        $this->db->where('code',$code);
        $this->db->where('solution',$solution_db);
        $this->db->where('qno',$qno);
        $query = $this->db->get();
        return $query->result(); 
    }
    
    function get_total_rows($code, $solution_table){   

        $this->db->select('*');
        $this->db->from('top_value_db');
        $this->db->where('code',$code);
        $this->db->where('solution',$solution_table);
        $query = $this->db->get();
    ...
    function insert_data($data){   
        ...
        if($this->db->insert('top_value_db', $data)){
            return true;
        }
        else{
            return false;
    ...
    function get_data_by_code($code){   

        $this->db->select('*');
        $this->db->from('top_value_db');
        $this->db->where('code',$code);
        $this->db->limit(3);
        $this->db->order_by("per", "DESC");
        $query = $this->db->get();
        return $query->result();
    }
    
    function get_personality_data($solution_db ,$code){   
    ...
    function get_scoring_data($part ,$qn, $ans){  

        $this->db->select('*');
        $this->db->from('uce_part1_4_scoring');
    ...
    function disha_pref_id_count_new($code,$solution,$opt_name,$opt_value){
        $this->db->select("a.qno as pp_qno, b.qno 
        as pre_que_qno,b.cat,b.sub_cat,
        c.options,c.value,b.$opt_name,d.cat as pref_cat ,d.sub_cat as pref_sub_cat");
         $this->db->where(["a.code" => $code,"a.solution" => $solution,"c.value"=>$opt_value]);
         ...
         $this->query = $this->db->get("ppe_part1_test_details a");
          //echo  $this->db->last_query();
         //print_r($this->query->result_array());die();
         return $this->query->result_array();
    }
    
    function get_aptitude_data($sol_dbs ,$code){  
        
        //echo $sol_dbs."---".$code;die();

        $this->db->select('*');
        $this->db->from('ppe_part1_test_details');
        $this->db->where('solution',$sol_dbs);
        $this->db->where('code',$code);
        $query = $this->db->get();
        return $query->result();
    }
    
    function get_aptitude_q_data($v,$qno){  
    ...
    function get_user_data($code){  
        ...
        $this->db->select('*');
        $this->db->from('user_code_list');
        $this->db->where('code',$code);
        $query = $this->db->get();
        return $query->result();
    }
    
    function get_apt_score_data($class, $param, $s_type){  
        // echo "in model";
        // echo $class."---".$param."---".$s_type;die();
    
        $this->db->select('*');
        $this->db->from('uce_apt_calculation');
        $this->db->where('class',$class);
        $this->db->where('param',$param);
        $this->db->where('score >',$s_type);
        $this->db->limit(1);
        $query = $this->db->get();
        return $query->result();
    }
    
    function get_uce_cal_data($class, $param, $low_l){  

        $this->db->select('*');
        $this->db->from('uce_apt_calculation');
        $this->db->where('class',$class);
        $this->db->where('param',$param);
        //$this->db->where('fs >',$low_l);
        $this->db->limit(1);
        $query = $this->db->get();
        return $query->result();
    }
